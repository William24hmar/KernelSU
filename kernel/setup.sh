...
setup_kernelsu() {
    ...
    test -d "$GKI_ROOT/KernelSU" || git clone https://github.com/William24hmar/KernelSU && echo "[+] Repository cloned."
    cd "$GKI_ROOT/KernelSU"
    git stash && echo "[-] Stashed current changes."
    if [ "$(git status | grep -Po 'v\d+(\.\d+)*' | head -n1)" ]; then
        git checkout master-susfs && echo "[-] Switched to master-susfs branch."
    fi
